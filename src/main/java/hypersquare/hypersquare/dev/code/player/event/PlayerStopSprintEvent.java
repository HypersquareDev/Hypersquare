...
public class PlayerStopSprintEvent implements CancellableEvent {
    @Override
    public ItemStack item() {
        return new EventItem()
            .setMaterial(Material.CHAINMAIL_BOOTS)
            .setName(Component.text("Player Stop Sprint Event").color(Colors.PURPLE))
            .setDescription(
                Component.text("Executes code when a player"),
                Component.text("stops sprinting."))
            .setCancellable(true)
            .build()
            ;
    }
