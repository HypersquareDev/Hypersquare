...
public class ItemEventsMenu extends Gui {
    public ItemEventsMenu(Player player) {
        super(player, "itemEvents", "Item events", 5);
    }

    @Override
    public void onOpen(InventoryOpenEvent event) {
        for (String itemStackKey : ItemManager.registeredItems().keySet()) {
            if (itemStackKey.contains("player_event.item_events.")) {
