...
public class MovementEventsMenu extends Gui {
    public MovementEventsMenu(Player player) {
        super(player, "movementEvents", "Movement events", 5);
    }

    @Override
    public void onOpen(InventoryOpenEvent event) {
        for (String itemStackKey : ItemManager.registeredItems().keySet()) {
            if (itemStackKey.contains("player_event.movement_events.")) {
